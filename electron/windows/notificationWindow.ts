...
type NativeGlassModule = typeof import("@markckb/electron-liquid-glass");
let nativeGlass: NativeGlassModule | null = null;
if (!usesSystemNotifications) {
  try {
    const mod: NativeGlassModule = require("@markckb/electron-liquid-glass");
...
let glassPanel: import("@markckb/electron-liquid-glass").GlassPanel | null = null;
...
export async function showNotification(data: any) {
  ...
  if (isAiInsightNotification) {
    ...
    // 系统通知（如 "Lumina 准备就绪"）不是聊天消息，不应受会话白/黑名单影响
    const isSystemNotification = sessionId.startsWith("Lumina-");
