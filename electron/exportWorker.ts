...
process.env.Lumina_WORKER = '1'
if (config.resourcesPath) {
  process.env.WCDB_RESOURCES_PATH = config.resourcesPath
}
if (config.userDataPath) {
  process.env.LUMINA_USER_DATA_PATH = config.userDataPath
  process.env.LUMINA_CONFIG_CWD = config.userDataPath
}
process.env.Lumina_PROJECT_NAME = process.env.Lumina_PROJECT_NAME || 'Lumina'
...
async function runWeliveEngine() {
  ...
  const rawRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'Lumina-welive-raw-'))
