...
const TEMP_MARKER = '.Lumina-backup-temp'
...
export class BackupService {
  ...
  private cleanupStaleTempDirs(): void {
    ...
    try {
      for (const entry of readdirSync(root)) {
        if (!entry.startsWith('Lumina-backup-')) continue
  ...
  async createBackup(outputPath: string, options: BackupOptions = {}): Promise<{ success: boolean; filePath?: string; manifest?: BackupManifest; error?: string }> {
    ...
    try {
      ...
      stagingDir = await this.createTempDir('Lumina-backup-')
  ...
  async inspectBackup(archivePath: string): Promise<{ success: boolean; manifest?: BackupManifest; error?: string }> {
    ...
    try {
      ...
      extractDir = await this.createTempDir('Lumina-backup-inspect-')
      await tar.x({
        file: archivePath,
        cwd: extractDir,
      ...
      if (manifest?.type !== 'Lumina-db-snapshots' || manifest.version !== 1) {
  ...
  async restoreBackup(archivePath: string): Promise<{ success: boolean; inserted?: number; ignored?: number; skipped?: number; error?: string }> {
    ...
    try {
      ...
      extractDir = await this.createTempDir('Lumina-backup-restore-')
      await tar.x({
        file: archivePath,
        cwd: extractDir,
