...
export class KeyService {
  ...
  private ensureLoaded(): boolean {
    ...
    let dllPath = ''
    try {
      console.log('[keyService] ensureLoaded 开始，即将 require("koffi")')
      // require('koffi') 加载 native addon，如果 ABI 不兼容可能直接 segfault
      this.koffi = require('koffi')
      console.log('[keyService] require("koffi") 成功，版本:', this.koffi.version || 'unknown')
      dllPath = this.getDllPath()
      console.log('[keyService] wx_key.dll 路径:', dllPath)
      ...
      if (this.isNetworkPath(dllPath)) {
        console.log('[keyService] 检测到网络路径，本地化 DLL')
        dllPath = this.localizeNetworkDll(dllPath)
        console.log('[keyService] 本地化后路径:', dllPath)
      }

      console.log('[keyService] 即将执行 koffi.load(wx_key.dll)，路径:', dllPath)
      // koffi.load 加载 wx_key.dll，如果 DLL 依赖缺失或架构不匹配可能 segfault
      this.lib = this.koffi.load(dllPath)
      console.log('[keyService] koffi.load(wx_key.dll) 成功，开始声明函数')
      ...
      console.log('[keyService] 函数声明完成，初始化成功')
  ...
  private async pollDbKeyFromHook(
    ...
    while (Date.now() < deadline) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        let msg = this.decodeUtf8(statusBuffer)
        const level = levelOut[0] ?? 0
        if (msg) {
          // 改写 DLL 返回的提示文案，使其更清晰
          if (msg.includes('Hook安装成功') && msg.includes('现在登录微信')) {
            msg = 'Hook 安装成功，请现在退出微信并重新登录（不是锁定，是退出登录），Lumina 会在登录瞬间自动捕获密钥'
          }
          logs.push(msg)
  ...
  private isLoginRelatedText(value: string): boolean {
    ...
    if (!normalized) return false
    // 排除 DLL 的操作提示（动作描述），这些不是登录状态检测
    // 例如 "Hook安装成功，现在登录微信" 是提示用户操作，不是检测到未登录
    const actionPhrases = ['现在登录微信', '现在登录', 'hook安装成功']
    if (actionPhrases.some((phrase) => normalized.includes(phrase.replace(/\s+/g, '')))) {
      return false
    }
    const keywords = [
      '登录',
  ...
  async autoGetDbKey(
    ...
    console.log('[keyService] autoGetDbKey 开始，timeout:', timeoutMs)
    if (!this.ensureWin32()) return { success: false, error: '仅支持 Windows' }
    console.log('[keyService] ensureWin32 通过，调用 ensureLoaded')
    if (!this.ensureLoaded()) return { success: false, error: 'wx_key.dll 未加载' }
    console.log('[keyService] ensureLoaded 通过，调用 ensureKernel32')
    if (!this.ensureKernel32()) return { success: false, error: 'Kernel32 Init Failed' }
    console.log('[keyService] ensureKernel32 通过，开始查找微信进程')

    const logs: string[] = []
    const deadline = Date.now() + timeoutMs
    onStatus?.('正在查找微信进程...', 0)
    let pid = await this.findWeChatPid()
    console.log('[keyService] findWeChatPid 返回:', pid)
    if (!pid) {
    ...
    while (pid && this.getRemainingMs(deadline) > 0) {
      ...
      console.log('[keyService] 调用 InitializeHook，PID:', pid)
      const ok = this.initHook(pid)
      console.log('[keyService] InitializeHook 返回:', ok)
      if (!ok) {
        if (!await this.isWeChatPidActive(pid)) {
    ...
    return {
      success: false,
      error: '获取密钥超时。Hook 机制需要在微信登录瞬间捕获密钥，如果微信已处于登录状态，请先完全退出微信（不是锁定，是退出登录），然后重新打开微信并完成登录，期间 Lumina 会自动捕获密钥。',
      logs
    }
  }
