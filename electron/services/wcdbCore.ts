...
export class WcdbCore {
  ...
  async initialize(): Promise<boolean> {
    ...
    try {
      ...
      try {
        ...
        for (const resPath of resourcePaths) {
          try {
            ...
            console.log(`[WCDB] InitProtection path=${resPath} rc=${protectionCode}`)
            if (protectionCode === 0) {
              protectionOk = true
              break
            }
            ...
            console.log(`[WCDB] InitProtection exception path=${resPath}: ${String(e)}`)
        ...
        if (!protectionOk) {
          ...
          console.log(`[WCDB] InitProtection 最终失败 finalCode=${finalCode}`)
          // -1006/-101/-102 是 DLL 内部的签名/授权校验失败。
          // 当资源文件与 WeFlow 完全相同时（已对比确认），这些错误码通常是
          // DLL 对调用进程身份做了校验，而非真正缺失文件。跳过保护检查
          // 不影响数据库读写操作，仅影响 DLL 内部的防盗用机制。
          if (finalCode === -1006 || finalCode === -101 || finalCode === -102) {
            this.writeLog(`[bootstrap] InitProtection returned ${finalCode} (signature/auth check), skipping protection and continuing`, true)
            console.warn(`[WCDB] InitProtection 返回 ${finalCode}（签名/授权校验），跳过保护检查继续初始化`)
          } else {
          ...
          console.log(`[WCDB] InitProtection 成功`)
        }
      } catch (e) {
      ...
      // 初始化
      // 设置环境变量，可能有助于绕过网络授权检查
      // DLL 内部可能检查 WEFLOW_SNAPSHOT_V1 / weflow_read_opt 等标识
      if (!process.env.WEFLOW_SNAPSHOT_V1) {
        process.env.WEFLOW_SNAPSHOT_V1 = '1'
      }
      // weflow_read_opt 可能是 "read-only/offline mode" 的开关
      // 设置为 1 可能跳过网络授权（api.weflow.top 当前不可访问）
      if (!process.env.weflow_read_opt) {
        process.env.weflow_read_opt = '1'
      }
      console.log(`[WCDB] 即将调用 wcdb_init()，进程名: ${process.argv0}, WEFLOW_SNAPSHOT_V1=${process.env.WEFLOW_SNAPSHOT_V1}, weflow_read_opt=${process.env.weflow_read_opt}`)
      const initResult = this.wcdbInit()
      console.log(`[WCDB] wcdb_init() 返回 ${initResult}`)
      this.writeLog(`[bootstrap] wcdb_init returned ${initResult} (WEFLOW_SNAPSHOT_V1=${process.env.WEFLOW_SNAPSHOT_V1}, weflow_read_opt=${process.env.weflow_read_opt})`, true)
      if (initResult !== 0) {
        // -1006 是签名/授权校验失败（可能因网络授权超时或进程身份校验）。
        // WeFlow 和 Lumina 使用相同的 DLL，数据库读写操作本身不依赖授权检查，
        // 尝试继续初始化，后续 wcdbOpenAccount 会验证数据库是否真正可用。
        if (initResult === -1006 || initResult === -101 || initResult === -102) {
          console.warn(`[WCDB] wcdb_init() 返回 ${initResult}（签名/授权校验），尝试继续初始化`)
          this.writeLog(`[bootstrap] wcdb_init returned ${initResult}, attempting to continue anyway`, true)
          // 尝试获取 DLL 内部日志，了解具体失败原因
