      ...
      if (missing.length > 0) {
        ...
        if (missingAvatars.length > 0) {
          ...
          for (const username of missingAvatars) {
            ...
            if (avatarUrl) {
              // 升级 http→https，避免 CSP 拦截
              const finalUrl = avatarUrl.startsWith('http://') ? 'https://' + avatarUrl.substring(7) : avatarUrl
              result[username].avatarUrl = finalUrl
              const cached = this.avatarCache.get(username)
              if (cached && this.shouldPersistAvatarUrl(finalUrl)) {
                cached.avatarUrl = finalUrl
