...
const isWorkerThread = process.env.LUMINA_WORKER === '1'
if (!isWorkerThread) {
  try {
...
export class ConfigService {
  ...
  constructor() {
    ...
    const storeOptions: any = {
      name: 'Lumina-config',
      defaults,
      projectName: String(process.env.LUMINA_PROJECT_NAME || 'Lumina').trim() || 'Lumina'
    }
    const runningInWorker = process.env.LUMINA_WORKER === '1'
    if (runningInWorker) {
      const cwd = String(process.env.LUMINA_CONFIG_CWD || process.env.LUMINA_USER_DATA_PATH || '').trim()
      if (cwd) {
    ...
    try {
      ...
      if (message.includes('projectName')) {
        const fallbackOptions = {
          ...storeOptions,
          projectName: 'Lumina',
          cwd: storeOptions.cwd || process.env.LUMINA_CONFIG_CWD || process.env.LUMINA_USER_DATA_PATH || process.cwd()
        }
   ...
   * 获取账号目录的真实绝对路径。
   *
   * 这是 Lumina 统一的账号目录解析入口，所有服务都应通过本方法获取
  ...
  private getUserDataPath(): string {
    const workerUserDataPath = String(process.env.LUMINA_USER_DATA_PATH || process.env.LUMINA_CONFIG_CWD || '').trim()
