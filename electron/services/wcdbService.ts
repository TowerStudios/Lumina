...
export class WcdbService {
  ...
  /**
   * 发送消息到 Worker 并等待响应
   * 对 'open' 和 'testConnection' 操作添加超时，避免 InitProtection 网络阻塞导致主进程挂起
   */
  private callWorker<T>(type: string, payload: any = {}): Promise<T> {
    ...
    return new Promise((resolve, reject) => {
      ...
      // open/testConnection 可能因 InitProtection 网络授权超时而阻塞，设 30s 超时
      const needsTimeout = type === 'open' || type === 'testConnection'
      ...
      if (needsTimeout) {
        timer = setTimeout(() => {
          ...
          if (p) {
            this.pending.delete(id)
            p.reject(new Error(`WCDB ${type} 操作超时（30s），可能是网络授权检查阻塞`))
          }
        }, 30000)
      }
    })
