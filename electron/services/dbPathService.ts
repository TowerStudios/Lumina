...
export class DbPathService {
  ...
  private parseGlobalConfig(rootPath: string): { wxid: string, nickname: string, avatarUrl: string } | null {
    try {
      ...
      // 将 http:// 升级为 https://，避免 CSP 拦截并提升安全性
  ...
  async autoDetect(): Promise<{ success: boolean; path?: string; error?: string }> {
    try {
      ...
      if (process.platform === 'darwin') {
        ...
        // Windows 微信4.x 数据目录
        // 1. 默认 Documents 路径（用户主目录下）
        possiblePaths.push(join(home, 'Documents', 'xwechat_files'))
        // 2. OneDrive 重定向路径
        possiblePaths.push(join(home, 'OneDrive', 'Documents', 'xwechat_files'))
        possiblePaths.push(join(home, 'OneDrive', '桌面', 'xwechat_files'))
        // 3. 旧版微信数据目录
        possiblePaths.push(join(home, 'Documents', 'WeChat Files'))
        possiblePaths.push(join(home, 'Documents', 'Weixin Files'))
        // 4. 扫描所有盘符根目录和 Documents 目录（用户可能重定向 Documents 到其他盘）
        for (const drive of ['C', 'D', 'E', 'F', 'G']) {
          const driveRoot = `${drive}:\\`
          if (existsSync(driveRoot)) {
            possiblePaths.push(join(driveRoot, 'xwechat_files'))
            possiblePaths.push(join(driveRoot, 'WeChat Files'))
            possiblePaths.push(join(driveRoot, 'Weixin Files'))
            possiblePaths.push(join(driveRoot, 'Documents', 'xwechat_files'))
            possiblePaths.push(join(driveRoot, 'Documents', 'WeChat Files'))
            possiblePaths.push(join(driveRoot, 'Documents', 'Weixin Files'))
          }
        }
        // 5. 从注册表读取微信安装路径，推导数据目录
        try {
          const regPath = this.readWeChatDataPathFromRegistry()
          if (regPath) possiblePaths.push(regPath)
        } catch { }
      }
  ...
  scanWxids(rootPath: string): WxidInfo[] {
    ...
    const globalInfo = this.parseGlobalConfig(resolvedRootPath);
    if (globalInfo) {
      console.log('[dbPathService] globalConfig 解析结果:', {
        wxid: globalInfo.wxid,
        nickname: globalInfo.nickname,
        avatarUrl: globalInfo.avatarUrl ? `${globalInfo.avatarUrl.substring(0, 60)}...` : '(空)'
      })
      for (const w of sorted) {
        if (w.wxid.startsWith(globalInfo.wxid) || sorted.length === 1) {
          w.nickname = globalInfo.nickname;
      ...
      console.log('[dbPathService] parseGlobalConfig 返回 null，无法提取头像和昵称')
    }
    return sorted;
  }

  /**
   * 从注册表读取微信文件保存路径（Windows 专用）
   * 微信 4.x 在 HKCU\Software\Tencent\Weixin 下保存 FileSavePath
   * 微信 3.x 在 HKCU\Software\Tencent\WeChat 下保存 FileSavePath
   */
  private readWeChatDataPathFromRegistry(): string | null {
    if (process.platform !== 'win32') return null
    try {
      // 动态 require 避免在 macOS 上加载 win32 模块
      const koffi = require('koffi')
      const advapi32 = koffi.load('advapi32.dll')
      const HKEY = koffi.alias('HKEY', 'intptr_t')
      const HKEY_PTR = koffi.pointer(HKEY)
      const RegOpenKeyExW = advapi32.func('RegOpenKeyExW', 'long', [HKEY, 'uint16*', 'uint32', 'uint32', koffi.out(HKEY_PTR)])
      const RegQueryValueExW = advapi32.func('RegQueryValueExW', 'long', [HKEY, 'uint16*', 'uint32*', 'uint32*', 'uint8*', 'uint32*'])
      const RegCloseKey = advapi32.func('RegCloseKey', 'long', [HKEY])
      ...
      const valueNames = ['FileSavePath', 'InstallPath']

      for (const regKey of regKeys) {
        const subKeyBuf = Buffer.from(regKey + '\0', 'ucs2')
        const phkResult = Buffer.alloc(8)
        if (RegOpenKeyExW(HKEY_CURRENT_USER, subKeyBuf, 0, KEY_READ, phkResult) !== ERROR_SUCCESS) continue
        const hKey = koffi.decode(phkResult, 'uintptr_t')

        try {
          for (const valueName of valueNames) {
            const valueNameBuf = Buffer.from(valueName + '\0', 'ucs2')
            const lpcbData = Buffer.alloc(4)
            lpcbData.writeUInt32LE(0, 0)
            if (RegQueryValueExW(hKey, valueNameBuf, null, null, null, lpcbData) !== ERROR_SUCCESS) continue
            const size = lpcbData.readUInt32LE(0)
            if (size === 0) continue
            const dataBuf = Buffer.alloc(size)
            if (RegQueryValueExW(hKey, valueNameBuf, null, null, dataBuf, lpcbData) !== ERROR_SUCCESS) continue
            let path = dataBuf.toString('ucs2')
            if (path.endsWith('\0')) path = path.slice(0, -1)
            if (!path) continue

            // FileSavePath 直接就是数据目录
            if (valueName === 'FileSavePath' && existsSync(path)) {
              // 微信 4.x 用 xwechat_files，微信 3.x 用 WeChat Files
              if (existsSync(join(path, 'xwechat_files'))) return join(path, 'xwechat_files')
              if (existsSync(join(path, 'WeChat Files'))) return join(path, 'WeChat Files')
              return path
            }
            // InstallPath 下找 xwechat_files 或 WeChat Files
